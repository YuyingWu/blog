...
图2.2展示了两个不同大小的盒子。盒子内的font-size、padding和border-radius各不相同。
...
在之前的例子里，我们计算到字号大小是19.2px（继承的16px乘以1.2em）。图2.4是相同的slogan元素，但有额外的1.2em padding以及为了让padding大小更加明显的灰色背景。可以看出，padding比font-size稍微大一些，尽管它俩声明的时候em值是一样的。
